...
import type { AnimalsController } from '../controllers/animals-controller.ts';
import { validateBody, validateId } from '../../middleware/validations.ts';
import { AnimalCreateSchema, AnimalUpdateSchema } from '../schemas/animal.ts';
...
export class AnimalsRouter {
    ...
    constructor(controller: AnimalsController) {
        ...
        this._router.get(
            '/',
            this.controller.getAllAnimals.bind(this.controller),
        );
        this._router.get(
            '/:id',
            validateId(),
